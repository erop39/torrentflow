...
Verification: run backend pytest with plugin autoload disabled (26 tests at the latest checkpoint); run the Vite production build. Alembic revisions currently run through `20260825_06`; API startup applies them idempotently. GitHub Actions builds backend/frontend images for linux/amd64 and linux/arm64, publishing `main` and version tags to GHCR. Git remote `origin` tracks `https://github.com/erop39/torrentflow.git` on `main`.
