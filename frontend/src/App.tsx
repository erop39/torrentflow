...
import { api } from './api'
import type { AuditEvent, Category, DownloadItem, Feed, FeedCheckResult, IntegrationStatus, Rule, ServiceHealth } from './types'
...
function Dashboard({ addToast, health, releases, loading, error, categories }: { addToast: (message: string) => void; health: ServiceHealth[]; releases: Release[]; loading: boolean; error: string | null; categories: Category[] }) {
  return <>
    <section className="health-grid" aria-label="Service health">
      {loading ? <div className="card loading-card">Loading service health…</div> : health.map(service => <div className="health-chip" key={service.name}><i className={service.status === 'healthy' ? 'status-dot' : 'status-dot warning'} /><div><strong>{service.name} · {service.status === 'healthy' ? 'Healthy' : 'Degraded'}</strong><small>{service.detail}</small></div></div>)}
    </section>
...
function FeedsPage({ feeds, loading, addToast, onCreated, onDeleted }: { feeds: Feed[]; loading: boolean; addToast: (message: string) => void; onCreated: (feed: Feed) => void; onDeleted: (feedId: number) => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); setSaving(true); setFormError(null); try { const feed = await api.createFeed({ name: String(data.get('name')), url: String(data.get('url')), adapter_type: 'generic_rss', interval_minutes: Number(data.get('interval')) }); onCreated(feed); setFormOpen(false); addToast('RSS feed added') } catch { setFormError('Could not save feed. Check the URL and API connection.') } finally { setSaving(false) } }
  ...
  return <article className="card releases-card"><div className="card-heading"><div><span className="eyebrow">Source monitoring</span><h2>RSS feeds</h2></div><Button primary onClick={() => setFormOpen(true)}><Plus /> Add feed</Button></div><p className="screen-copy">Sources stay enabled through transient errors. Review health, schedule and next permitted check.</p>{formOpen && <form className="feed-form" onSubmit={submit}><label>Name<input name="name" required placeholder="e.g. TorrentLeech" /></label><label>RSS URL<input name="url" type="url" required placeholder="https://…" /></label><label>Interval<select name="interval" defaultValue="30"><option value="10">10 minutes</option><option value="30">30 minutes</option><option value="60">60 minutes</option></select></label><div className="form-actions"><Button onClick={() => setFormOpen(false)}>Cancel</Button><Button primary>{saving ? 'Saving…' : 'Save feed'}</Button></div>{formError && <p className="form-error">{formError}</p>}</form>}{loading ? <div className="empty">Loading RSS feeds…</div> : feeds.length ? <div className="list">{feeds.map(feed => <div key={feed.id}><div className="data-row"><div className="release-name"><strong>{feed.name}</strong><small>{feed.adapter_type.replace('_', ' ')} · {feed.url}</small></div><span>{feed.interval_minutes} min</span><span>Enabled</span><span>—</span><Badge warning={!feed.enabled}>{feed.enabled ? 'Healthy' : 'Paused'}</Badge><div className="feed-actions"><Button onClick={() => check(feed)}><RefreshCw /> Check now</Button><Button label={`Delete ${feed.name}`} onClick={() => remove(feed)}><Trash2 /> Delete</Button></div></div>{scanResults[feed.id] && <FeedScanResult result={scanResults[feed.id]} />}</div>)}</div> : <div className="empty"><strong>No RSS feeds yet</strong><br />Add a source to start evaluating releases.</div>}</article>
}
...
function RulesPage({ rules, onCreated, addToast, categories }: { rules: Rule[]; onCreated: (rule: Rule) => void; addToast: (message: string) => void; categories: Category[] }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const rule = await api.createRule({ name: String(data.get('name')), include_keywords: String(data.get('keywords')), min_seeds: Number(data.get('seeds')), action: String(data.get('action')) as Rule['action'], priority: Number(data.get('priority')), category: String(data.get('category')) })
...
export function App() {
  ...
  return <div className="app-shell"><aside className="rail"><div className="brand" aria-label="TorrentFlow"><Waves /></div><nav aria-label="Primary navigation">{pages.map(({ id, label, icon: Icon }) => <button key={id} className={page === id ? 'active' : ''} type="button" title={label} aria-label={label} aria-current={page === id ? 'page' : undefined} onClick={() => setPage(id)}><Icon /></button>)}</nav></aside>
    ...
    <nav className="mobile-nav" aria-label="Mobile navigation">{pages.slice(0, 4).concat(pages[7]).map(({ id, label, icon: Icon }) => <button key={id} type="button" className={page === id ? 'active' : ''} aria-current={page === id ? 'page' : undefined} onClick={() => setPage(id)}><Icon /><span>{label}</span></button>)}</nav>
