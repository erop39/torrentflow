...
Current focus: release handoff after final static QA. Categories are persistent and selectable by rules; Settings controls their colors and default visibility, while credentials remain environment-only. The schema lifecycle is Alembic-only in local and production; multi-arch image CI and Synology deployment documentation are complete.
