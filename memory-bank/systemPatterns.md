...
- RSS ingestion is resilient: title parsing is presentation-only with an unknown fallback, while each scan writes a bounded, secret-free success/failure job record. Telegram templates are non-secret portable settings and may use only explicit scalar placeholders.
