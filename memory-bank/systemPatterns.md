...
- Feed adapters may use a validated per-feed HTTP(S)/SOCKS5 proxy. TorrentLeech extraction normalizes uploader, freeleech, double-upload, and size metadata for matching; matching qBittorrent actions can set a per-rule category and save path.
...
- SQLite does not persist passkeys, session cookies, or integration credentials. The browser session is a signed HttpOnly cookie containing only the admin flag; credentials remain environment-only.
