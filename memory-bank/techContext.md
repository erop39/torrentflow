...
Backend: Python 3.12+, FastAPI, SQLAlchemy async, SQLite/aiosqlite, Alembic, httpx (with SOCKS support), feedparser, guessit, PyYAML, cryptography/Fernet, uvicorn.
...
Verification: run backend pytest with plugin autoload disabled; run the Vite production build. Alembic revisions currently run through `20260826_12`; API startup applies them idempotently. GitHub Actions builds backend/frontend images for linux/amd64 and linux/arm64, publishing `main` and version tags to GHCR. Git remote `origin` tracks `https://github.com/erop39/torrentflow.git` on `main`.
